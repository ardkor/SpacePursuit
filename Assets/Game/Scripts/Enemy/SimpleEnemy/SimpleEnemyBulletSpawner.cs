...
public class SimpleEnemyBulletSpawner : ObjectPool
{
    ...
    public void SpawnBullet(Vector3 spawnPoint, float angle, Vector3 direction)
    {
        if (TryGetObject(out GameObject bullet))
        {
            ...
            _currentBullet.SetRotation(new Vector2 (_playerTransform.position.x, _playerTransform.position.y), angle, direction);
        }
    }
}
